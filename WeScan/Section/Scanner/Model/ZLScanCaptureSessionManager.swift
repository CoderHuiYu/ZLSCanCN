//
//  ZLScanCaptureSessionManager.swift
//  WeScan
//
...
protocol ZLScanRectangleDetectionDelegateProtocol: NSObjectProtocol {
...
final class CaptureSessionManager: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    private let rectangleFunnel = ZLScanRectangleFeaturesFunnel()
    weak var delegate: ZLScanRectangleDetectionDelegateProtocol?
    ...
    init?(videoPreviewLayer: AVCaptureVideoPreviewLayer) {
        ...
        guard let inputDevice = AVCaptureDevice.default(for: AVMediaType.video),
            ...
            captureSession.canAddOutput(videoOutput) else {
                let error = ZLScanImageScannerControllerError.inputDevice
    ...
    internal func start() {
        ...
        default:
            let error = ZLScanImageScannerControllerError.authorization
            delegate?.captureSessionManager(self, didFailWithError: error)
        }
    }
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        if #available(iOS 11.0, *) {
            ZLScanVisionRectangleDetector.rectangle(forImage: finalImage) { (rectangle) in
                self.processRectangle(rectangle: rectangle, imageSize: imageSize)
            }
        } else {
            ZLScanCIRectangleDetector.rectangle(forImage: finalImage) { (rectangle) in
                self.processRectangle(rectangle: rectangle, imageSize: imageSize)
            }
        }
    }
    ...
    private func setImageOrientation() {
        ...
        motion.startAccelerometerUpdates(to: OperationQueue()) { data, error in
            ...
            if data.acceleration.x >= motionThreshold {
                ZLScanCaptureSession.current.editImageOrientation = .left
            } else if data.acceleration.x <= -motionThreshold {
                ZLScanCaptureSession.current.editImageOrientation = .right
            } else {
    ...
    private func processRectangle(rectangle: ZLQuadrilateral?, imageSize: CGSize) {
        if ZLScanCaptureSession.current.isPreviewing {
            return
        }
        if let rectangle = rectangle {
            ...
            self.rectangleFunnel.add(rectangle, currentlyDisplayedRectangle: self.displayedRectangleResult?.rectangle) { [weak self] (result, rectangle, currentAutoScanPassCount) in
                ...
                if startShootLoading, ZLScanCaptureSession.current.autoScanEnabled, !ZLScanCaptureSession.current.isEditing {
    ...
    @discardableResult private func displayRectangleResult(rectangleResult: RectangleDetectorResult) -> ZLQuadrilateral {
...
extension CaptureSessionManager: AVCapturePhotoCaptureDelegate {
    ...
    func photoOutput(_ captureOutput: AVCapturePhotoOutput, didFinishProcessingPhoto photoSampleBuffer: CMSampleBuffer?, previewPhoto previewPhotoSampleBuffer: CMSampleBuffer?, resolvedSettings: AVCaptureResolvedPhotoSettings, bracketSettings: AVCaptureBracketedStillImageSettings?, error: Error?) {
        ...
        if let sampleBuffer = photoSampleBuffer,
            ...
            let error = ZLScanImageScannerControllerError.capture
            delegate?.captureSessionManager(self, didFailWithError: error)
            return
        }
    ...
    private func completeImageCapture(with imageData: Data) {
        DispatchQueue.global(qos: .background).async { [weak self] in
            ZLScanCaptureSession.current.isEditing = true
