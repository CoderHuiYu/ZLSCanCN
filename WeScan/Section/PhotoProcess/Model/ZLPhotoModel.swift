...
extension ZLPhotoModel {
    func save(handle:((_ isSuccess: Bool)->())) {
        ...
        if let array = NSMutableArray(contentsOfFile: kZLScanPhotoModelDataPath) {
            array.add(dict)
    ...
    func replace(_ originalImage: UIImage, _ scannedImage: UIImage, _ enhancedImage: UIImage, _ isEnhanced: Bool,_ detectedRect: ZLQuadrilateral, handle:@escaping ((_ isSuccess: Bool, _ changedModel: ZLPhotoModel?)->())) {
        if let array = NSMutableArray(contentsOfFile: kZLScanPhotoModelDataPath) {
            var index: Int = 0
            for dict in array {
                guard let dict = dict as? [String: Any] else {
    ...
    static func getAllModel(handle:((_ isSuccess: Bool,_ models: [ZLPhotoModel]?)->())) {
        guard let array = NSArray(contentsOfFile: kZLScanPhotoModelDataPath) else {
            handle(false, nil)
            return
        }
    ...
    static func removeAllModel(handle:((_ isSuccess: Bool)->())) {
        ...
        if manager.fileExists(atPath: kZLScanPhotoModelDataPath) {
            do {
                try manager.removeItem(atPath: kZLScanPhotoModelDataPath)
